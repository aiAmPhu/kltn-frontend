        ...
        <div className="max-w-3xl mx-auto mt-24 p-6 bg-white rounded-lg shadow-md">
            <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-600">MSSV: {user.userId}</p>
            ...
            {detail && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        <strong>Họ tên:</strong> {detail.lastName} {detail.firstName}
                    </div>
                    <div>
                        <strong>Giới tính:</strong> {detail.gender}
                    </div>
                    <div>
                        <strong>Ngày sinh:</strong> {dayjs(detail.birthDate).format("DD/MM/YYYY")}
                    </div>
                    <div>
                        <strong>Nơi sinh:</strong> {detail.birthPlace}
                    </div>
                    <div>
                        <strong>Số điện thoại:</strong> {detail.phone}
                    </div>
                    <div>
                        <strong>Email cá nhân:</strong> {detail.email}
                    </div>
                    <div>
                        <strong>Email phụ huynh:</strong> {detail.parentEmail}
                    </div>
                    <div>
                        <strong>Số CCCD:</strong> {detail.idNumber}
                    </div>
                    <div>
                        <strong>Ngày cấp CCCD:</strong> {dayjs(detail.idIssueDate).format("DD/MM/YYYY")}
                    </div>
                    <div>
                        <strong>Nơi cấp CCCD:</strong> {detail.idIssuePlace}
                    </div>
                    <div>
                        <strong>Địa chỉ thường trú:</strong>{" "}
                        {`${detail.houseNumber} ${detail.streetName}, ${detail.commune}, ${detail.district}, ${detail.province}`}
                    </div>
                    <div>
                        <strong>Trạng thái hồ sơ:</strong>{" "}
                        {detail.status === "waiting" ? "Đang chờ duyệt" : detail.status}
                    </div>
                </div>
            )}
        </div>
