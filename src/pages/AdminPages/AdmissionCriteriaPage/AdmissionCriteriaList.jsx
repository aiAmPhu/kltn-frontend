...
const AdmissionCriteriaList = ({ criterias = [], setCriterias }) => {
    ...
    const loadAdmissionCriteria = async () => {
        ...
        try {
            ...
            setError(error.response?.data?.message || "Lỗi khi tải danh sách diện xét tuyển");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDelete = async (criteria) => {
        const confirmDelete = window.confirm(`Bạn có chắc chắn muốn xóa diện ${criteria.criteriaName}?`);
        if (confirmDelete) {
            try {
                const token = localStorage.getItem("token");
                await axios.delete(`${API_BASE_URL}/adcs/delete/${criteria.criteriaId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const filteredCriteria = criterias.filter((criteria) =>
    ...
    return (
        <div className="w-full bg-white shadow-md">
            <div className="p-6">
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-blue-600 text-center">Quản lý Diện xét tuyển</h1>
                </div>
                                ...
                                {isLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-4 text-center text-gray-500">
                                            Không có diện xét tuyển phù hợp
                                        </td>
                                    </tr>
                                ) : (
                                    currentItems.map((criteria) => (
                                        <tr key={criteria.criteriaId} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{criteria.criteriaId}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-500">{criteria.criteriaName}</div>
                                            </td>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mt-4">
                    <div className="flex items-center mb-4 md:mb-0">
                        ...
                        <span className="text-sm text-gray-700">
                            Tổng: <span className="font-medium">{filteredCriteria.length}</span> diện xét tuyển
                        </span>
                    </div>
