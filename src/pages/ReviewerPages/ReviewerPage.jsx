...
const ReviewerPage = () => {
    ...
    const [reviewerMajors, setReviewerMajors] = useState([]);
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            try {
                ...
                const usersRes = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/users/getForReviewer`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                const users = usersRes.data.data.users || [];
                const reviewerMajors = usersRes.data.data.reviewerMajors || [];

                setReviewerMajors(reviewerMajors);
                ...
                // Lấy thông tin bổ sung cho từng user
                const [transcriptsRes, learningRes, photosRes, informationRes] = await Promise.all([
                ...
                // Map với status từ các service khác
                const usersWithStatus = users.map((user) => {
                    const transcriptData = transcriptsRes.data.data.find((t) => t.userId === user.userId);
                    const learningData = learningRes.data.data.find((l) => l.userId === user.userId);
                    const photo = photosRes.data.data.find((p) => p.userId === user.userId);
                    const info = informationRes.data.find((i) => i.userId === user.userId);

                    return {
                        ...user,
                        transcriptStatus: transcriptData?.status || "waiting",
    ...
    const getFilteredUsers = () => {
        ...
        if (searchTerm.trim() !== "") {
            ...
            result = result.filter((user) => {
                ...
                return (name && name.includes(search)) || (userIdStr && userIdStr.includes(search));
            });
        }
                ...
                <div className="w-full max-w-7xl mx-auto px-2 sm:px-8 py-8">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col">
                        ...
                        <div className="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                            ...
                            {reviewerMajors.length > 0 && (
                                <p className="text-sm text-gray-600 mt-1">
                                    Mã ngành quản lý: {reviewerMajors.join(", ")}
                                </p>
                            )}
                            <button
                                            ...
                                            <div className="space-y-3">
                                                {currentItems.length === 0 ? (
                                                    ...
                                                    currentItems.map((user) => (
                                                        <div
                                                            key={user.userId}
                                                            onClick={() => navigate(`/reviewer/user/${user.userId}`)}
